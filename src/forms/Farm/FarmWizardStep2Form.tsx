...
import {div, multiple, number, plus} from "../../libs/math"
...
const FarmWizardStep2Form = ({
  ...
  const maxLiquidity = plus(findBalanceFn?.(token1) ?? "0", transactions[1] ?? "0")
      ...
      <section>
        <div className={styles.multiLinesForm}>
          ...
          <div className={styles.full_submit}>
            <button className={styles.button} type='button' onClick={stepForward} disabled={disabled} >WITHDRAW LP AND TX PROFIT</button>
          </div>
        </div>
      </section>
    </MiniFormContainer>
